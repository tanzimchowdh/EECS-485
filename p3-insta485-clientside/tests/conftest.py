...
class LiveServer:
    ...
    def stop(self):
        ...
        if self.thread.is_alive():
            # Thread has failed to join with the main thread (e.g. join has
            # timed out), so we need to forcibly terminate it using the
            # protected stop method. This method is protected/hidden because a
            # forceful thread termination may lead to corrupted data or leaks.
